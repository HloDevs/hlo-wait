create table if not exists waitlist_accounts (id uuid primary key default gen_random_uuid(), email text not null unique, handle text unique check (handle is null or handle ~ '^[a-z0-9]{1,30}$'), verified_at timestamptz, created_at timestamptz not null default now());
